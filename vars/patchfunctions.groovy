...
def stage(target,toState,patchConfig,task, Closure callBack) {
	...
	def nop = !skip && patchConfig.mavenArtifacts.empty && patchConfig.dbObjects.empty && !patchConfig.installJadasAndGui && (!patchConfig.installDockerAndWindowsServices || patchConfig.installDockerAndWindowsServices == null) && !["Approve","Notification"].contains(task)
	log("skip = ${skip}","stage")
	log("nop  = ${nop}","stage")
